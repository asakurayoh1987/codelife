...
2. 异常断点
   ...
   vscode的`RUN AND DEBUG`面板中也有对应地方

   ![image-20221121085058885](../media/image-20221121085058885.png)

3. 条件断点

   在vscode或chrome中，右键代码所在的行可以看到此选项，可以填入表达式，当表达式为`true`时断住
...
5. DOM变更断点
   ...
   右键DOM元素可查看菜单，分别表示DOM子树修改、DOM元素属性修改以及节点被移除时断点
...
7. url请求断点
   ...
   `source`面板右侧的`XHR/fetch Breakpoints`，可以填入URL或部分字符串，如果为空则表示在任务URL请求时都断住
...
从开始加载网页到第一个文本、图像、svg、非白色的canvas渲染完成的耗时
...
最大的内容（文字/图片）的渲染时间，计算方式是从网页开始渲染到渲染完成，每次渲染内容都对比下大小，如果是更大的内容就更新下LCP的值：
...
与前端的FCP、LCP不同，FMP记录的是关键内容的渲染时间，比如视频网站，渲染视频最关键，别的内容不重要。
...
然后通过`PerformanceObserver`拿到它的渲染时间，就是FMP的值：
...
即我们熟知的`DOMContentLoaded`事件，html文档完全被加载和解析完成时触发，无需要等待stylesheet、img和iframe的加载完成
...
即我们熟悉的`load`事件，此时html加载和解析完成，并且它依赖的资源（iframe、img、stylesheet）也加载完成
...
- 从FCP后开始计算
- 持续5秒内无长任务（大于50ms）且无两个以上正在进行中的GET请求
...
Total Blocking Time，记录在FCP到TTI之间所有长任务的阻塞时间的总和
...
谷歌从上面的Web Vitals中选出了3个核心指标：LCP、FID、CLS，分别代表**加载性能**、**交互性能**、**视觉稳定性**

上述指标都可以通过chrome内嵌的lighthouse来测量，同时它还提供了一个可视化的[计算器](https://googlechrome.github.io/lighthouse/scorecalc/#FCP=780&SI=1743&FMP=723&TTI=1600&FCI=6500&LCP=2324&TBT=14&CLS=0&device=desktop&version=9)，用于量化这些指标值为多少时，得分是怎么来的
