...
## 三、关于x-real-ip与x-forwarded-for

在 Nginx 配置中，`X-Real-IP` 和 `X-Forwarded-For` 是两种常用的 HTTP 头部，用于在反向代理设置中传递原始客户端的 IP 地址信息。它们的值通常如下：

1. **X-Real-IP**:

  - `X-Real-IP` 通常被设置为**直接连接到 Nginx 服务器的客户端的 IP 地址**。在反向代理的情况下，这通常是上游代理（比如负载均衡器）的 IP 地址。

  - 例如，如果客户端的 IP 是 `123.45.67.89`，并且客户端直接连接到 Nginx，那么 `X-Real-IP` 将被设置为 `123.45.67.89`。

  - 在 Nginx 配置中，这个值通常是通过 `proxy_set_header X-Real-IP $remote_addr;` 设置的。

2. **X-Forwarded-For**:

  - `X-Forwarded-For` 用于记录所有经过的代理服务器的 IP 地址，包括客户端的原始 IP 地址。这个头部可以包含多个 IP 地址，由逗号分隔，按照客户端到服务器的顺序排列。

  - 例如，如果客户端的 IP 是 `123.45.67.89`，经过了两个代理服务器（IP 分别为 `98.76.54.32` 和 `76.54.32.10`），那么 `X-Forwarded-For` 将被设置为 `123.45.67.89, 98.76.54.32, 76.54.32.10`。

  - 在 Nginx 配置中，这个值通常是通过 `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` 设置的。



这两个头部对于理解客户端请求的实际来源非常重要，特别是在反向代理或负载均衡环境中。但是请注意，由于这些头部可以被客户端伪造，它们可能不总是完全可靠。因此，在使用这些头部信息进行安全决策时，需要小心谨慎。

如果`X-Forwarded-For`被设置为`123.45.67.89, 98.76.54.32, 76.54.32.10`，那么 `X-Real-IP`通常会被设置为直接连接到你的 Nginx 服务器的客户端的 IP 地址，即：`76.54.32.10`